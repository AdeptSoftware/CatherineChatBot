...
class TaskManager(core.thread.BaseThread):
	...
	def stop(self):
		with self._queue:
			self._queue.value.clear()
		super().stop()

	def append(self, fn, obj):
		with self._queue:
			self._queue.value += [(fn, obj)]

	def on_update(self):
		with self._queue:
			if len(self._queue.value) > 0:
				self._last = self._queue.value.pop(0)
...
class EventManager(core.thread.BaseThread):
	...
	def new_ex(self, name, fn, delay, cooldown, data=None):
		...
		with self._events:
			...
			self._events.value[name] = core.safe.SafeVariable({
				"fn":		fn,
				"data":		data,
				"cooldown":	cooldown,
	...
	def delete(self, name):
		with self._events:
			if name in self._events.value:
				self._events.value.pop(name)
				return True
		return False

	# Пример использования полученного события:
	# with event: event.value["fn"](event.value)
	def get(self, name):
		with self._events:
			if name in self._events.value:
				return self._events.value[name]
		return None
	...
	def keys(self):
		with self._events:
			return self._events.value.keys()
	...
	def on_update(self):
		...
		with self._events:
			for name in self._events.value:
				event = self._events.value[name]
				with event:
					if now >= event.value["next"]:
						if not event.value["fn"](event.value):
							# Удаляем события, которые вернули False
							deleted += [name]
						else:
							event.value["next"] = now+event.value["cooldown"]
			if deleted:
				for name in deleted:
					self._events.value.pop(name)
